...
DWORD WINAPI ClientThread(LPVOID lpParam)
{
    ...
    int ret;
    char buf[256];
    // 32 bit for name, 32 bit for password
    ...
    while (1)
    {
        ...
        else
        {
            ...
            if (found == 1)
            {
                const char* msg = "Sign in successfully. Write something\n";
                send(client, msg, strlen(msg), 0);
    ...
    while (1)
    {
        ...
        if (strcmp(cmd, "/all") == 0)    //group chat, if "/all", sending msg to all client  
        {
            // Send msg to other clients 
            // Avoid self sending
            for (int i = 0; i < numClients; i++)
            {
                char* msg = buf + strlen(cmd) + 1; // charaters after cmd are sent 
